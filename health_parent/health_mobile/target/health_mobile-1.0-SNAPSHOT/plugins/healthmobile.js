...
function getUrlParam(paraName) {
    ...
    //alert(url); // 此处的url为我们能在地址栏看到的详细的地址
    var arrObj = url.split("?");
    if (arrObj.length > 1) {
        var arrPara = arrObj[1].split("&");//id=3&name=jack
        var arr;
        for (var i = 0; i < arrPara.length; i++) {//[id=3, name=jack]
            arr = arrPara[i].split("=");// [id, 3],[name, jack]
            if (arr != null && arr[0] == paraName) {
                return arr[1];// 此时就取到了[3]
            }
        }
        return "";
    }
...
var nums = 30;
var validateCodeButton;// 倒计时按钮对象
//基于定时器实现30秒倒计时效果
function doLoop() {
